...
func (r *RTContainerCheck) Init(cfg *config.AgentConfig, sysInfo *pkg.SystemInfo) {
	r.sysInfo = sysInfo
}
...
func fmtContainerStats(
	...
	lastRates map[string]pkg.ContainerRateMetrics,
	lastRun time.Time,
	chunks int,
) [][]*pkg.ContainerStat {
	return make([][]*pkg.ContainerStat, chunks)
}
