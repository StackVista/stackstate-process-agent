...
task :imports do
  ...
  # goimports want file paths instead of package import paths. 
  # We want to use the exact file name because we want to exclude file generated by protobuf (`pb.go`).
  # packages = `go list -f '{{range .GoFiles}}{{$.Dir}}/{{.}}{{"\n"}}{{end}}' ./... | grep -v '/vendor/' | grep -v 'pb.go$'`.split("\n")
  packages = `go list -f '{{range .GoFiles}}{{$.Dir}}/{{.}}{{"\\n"}}{{end}}' ./... | grep -v '/vendor/' | grep -v 'pb.go$'`.split("\n")
  failure = false
  packages.each do |pkg|
    out = `goimports -d -w #{pkg}`
    errors = out.split("\n")
    if errors.length > 0
      # if there is at least one error we will fail at the end.
      failure = true
