...
llvm-objdump -S $WORKDIR/pkg/ebpf/bytecode/build/${ARCH}/usm-debug.o > $OUTPUTDIR/usm_debug.txt || true
llvm-objdump -S $WORKDIR/pkg/ebpf/bytecode/build/${ARCH}/usm.o > $OUTPUTDIR/usm.txt || true

echo "TESTING ARCH ${ARCH}"
cat $OUTPUTDIR/usm_debug.txt
cat $OUTPUTDIR/usm.txt

llvm-objdump -S $WORKDIR/pkg/ebpf/bytecode/build/${DD_ARCH}/usm-debug.o > $OUTPUTDIR/usm_debug.txt || true
llvm-objdump -S $WORKDIR/pkg/ebpf/bytecode/build/${DD_ARCH}/usm.o > $OUTPUTDIR/usm.txt || true
